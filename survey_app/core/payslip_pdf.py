...
def generate_payslip_pdf(nomina: dict, grupo_nombre: str = "") -> bytes:
    ...
    for c in conceptos:
        data.append([Paragraph(str(c.get("concepto", "")), H),
                     Paragraph(i18n.etiqueta(c.get("tipo", "")), H), Paragraph(_money(c.get("monto")), H)])
    ...
    if ap > 0:
        story += [Spacer(1, 6),
                  Paragraph(d("Employer contributions (not deducted from the employee)") + f": {_money(ap)}", sm)]
    story += [Spacer(1, 6),
              Paragraph(d("Generated by the app; statutory amounts must be validated."), sm)]
