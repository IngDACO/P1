...
Retorna la mejor combinación RL/FB y un log detallado de cada paso evaluado.
"""
import numpy as np
...
def optimize(survey_adjusted: list, limits: dict, params: dict) -> dict:
    ...
    best      = None
    step_log  = []   # log completo de cada combinación evaluada
    ...
    for rl in rl_steps:
        ...
        for fb in fb_steps:
            ...
            # Contar valores fuera de límite por columna
            off_by_col = {}
            total_off  = 0
            min_by_col = {}
            for col in cols:
            ...
            if wall and fs is not None and tsw < fs:
                ...
                if 0 <= stop_idx < len(modified):
                    ...
                    if wall_side == "R" and stop_row["OR"] < lim_map["OR"]:
                        wall_fail   = True
                        wall_reason = (f"Pared limitante: OR parada {wall_stop} = "
                                       f"{stop_row['OR']:.1f} < LIMIT OR={lim_map['OR']:.1f}")
                    if wall_side == "L" and stop_row["OL"] < lim_map["OL"]:
                        wall_fail   = True
                        wall_reason = (f"Pared limitante: OL parada {wall_stop} = "
            ...
            log_entry = {
                ...
                "min_by_col": {k: round(v,2) for k,v in min_by_col.items()},
                "wall_fail": wall_fail,
    ...
    return {"best": best, "step_log": step_log}
