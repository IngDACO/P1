...
def generate_report(project_params, calculated, survey_original,
    ...
    story += [
        ...
        _calc_block("LIMIT ZB (raw)",
            f"LIMIT ZB = SF{'1' if omega=='R' else '2'} × 0.3  "
            f"(Z está del lado {'L' if omega=='R' else 'R'}, opuesto al Omega)",
            f"{fs('SF1') if omega=='R' else fs('SF2')} × 0.3",
            f"LIMIT ZB raw = {lzb_raw:.2f} mm", styles), sp(4),
        Paragraph(
            f"<b>Omega lado {omega}  |  Z lado {'L' if omega=='R' else 'R'}:</b>  "
