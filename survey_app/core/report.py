...
def generate_report(project_params, calculated, survey_original,
    ...
    if omega == "R":
        lob, lzb, lr, ll = lor, lwl, lob_raw, lzb_raw
    else:
        lob, lzb, lr, ll = lol, lwr, lzb_raw, lob_raw
    ...
    story += [
        ...
        _calc_block("LIMIT OB (raw)", "LIMIT OB = (SG − (TG/2)) × 0.3", f"({fs('SG')} − ({fs('TG')}/2)) × 0.3", f"LIMIT OB raw = {lob_raw:.2f} mm", styles), sp(3),
        _calc_block("LIMIT ZB (raw)", "LIMIT ZB = SF2 × 0.3",            f"{fs('SF2')} × 0.3",           f"LIMIT ZB raw = {lzb_raw:.2f} mm", styles), sp(4),
        Paragraph(f"<b>Omega lado {omega}:</b>  LIMIT OB = {lob:.2f} mm  |  LIMIT ZB = {lzb:.2f} mm  |  LIMIT R = {lr:.2f} mm  |  LIMIT L = {ll:.2f} mm", styles["Normal2"]), sp(8),
    ]
    ...
    story += [
        ...
        _calc_block("Offset FR", "Offset FR = LIMIT FR − FRT",                    f"{lfr:.2f} − {frt:.2f}",                                                           f"Offset FR = {off_fr:.2f} mm", styles), sp(3),
        _calc_block("Offset FL", "Offset FL = LIMIT FL − FLT",                    f"{lfl:.2f} − {flt:.2f}",                                                           f"Offset FL = {off_fl:.2f} mm", styles), sp(3),
        _calc_block("Offset WR", "Offset WR = LIMIT WR − WRT + ((BSR−BS)/2)",     f"{lwr:.2f} − {wrt:.2f} + (({bsr:.2f}−{bs:.2f})/2) = {lwr:.2f} − {wrt:.2f} + {(bsr-bs)/2:.2f}", f"Offset WR = {off_wr:.2f} mm", styles), sp(3),
        _calc_block("Offset WL", "Offset WL = LIMIT WL − WLT + ((BSR−BS)/2)",     f"{lwl:.2f} − {wlt:.2f} + (({bsr:.2f}−{bs:.2f})/2) = {lwl:.2f} − {wlt:.2f} + {(bsr-bs)/2:.2f}", f"Offset WL = {off_wl:.2f} mm", styles), sp(8),
    ]
    ...
    # Pasos VÁLIDOS con detalle completo
    story += [Paragraph("Pasos válidos evaluados (detalle completo):", styles["SubHead2"]), sp(2)]
    ...
    for s in valid_steps:
        ...
        is_best = (s["rl"] == best_rl and s["fb"] == best_fb)
        estado  = Paragraph("✅ ÓPTIMO" if is_best else "", styles["Normal2"])
        row = [
    ...
    for ri, s in enumerate(valid_steps, start=1):
        if s["rl"] == best_rl and s["fb"] == best_fb:
            cmds_v.append(("BACKGROUND", (0,ri),(-1,ri), C_BEST))
            cmds_v.append(("FONTNAME",   (0,ri),(-1,ri), "Helvetica-Bold"))
        # Columnas con OFF > 0 en rojo
        obc = s.get("off_by_col",{})
    ...
    if best:
        story += [
            _calc_block("Combinación óptima seleccionada",
                "Criterio: menor número total de valores fuera de límite",
                f"RL = {best['rl']} mm  |  FB = {best['fb']} mm",
                f"Total valores fuera de límite: {best['total_off']}", styles, ok=(best["total_off"]==0)),
            sp(4),
            Paragraph("Matriz solución con desplazamientos aplicados:", styles["Note"]), sp(3),
        ]
        best_df  = pd.DataFrame(best["matrix"])
        best_min = {f"MIN_{c}": min(best_df[c]) for c in survey_cols}
        story += [_survey_table(best_df, lim_map, best_min, styles), sp(4)]
        final_sum = []
        for col in survey_cols:
            cv = [r[col] for r in best["matrix"]]
            final_sum.append({"Columna": col, "Límite (mm)": f"{lim_map[col]:.2f}",
                "Fuera límite": sum(1 for v in cv if v < lim_map[col]),
                "Mínimo (mm)": f"{min(cv):.2f}", "Dif vs Límite": f"{lim_map[col]-min(cv):.2f}"})
        story += [_summary_table(final_sum, styles), sp(8)]
    else:
